...
for y, x in enumerate(dag_updating):
    ax1.text(x + 6, y - 0.3*bar_width, '%s' % float('%.1f' % dag_updating[y]), ha='center', va='bottom', fontdict=text_font)
for y, x in enumerate(dag_matching):
    ax1.text(x + 6, y + 0.7*bar_width, '%s' % float('%.1f' % dag_matching[y]), ha='center', va='bottom', fontdict=text_font)
...
ax1.set_xlabel('CPU Clocks', label_font)
