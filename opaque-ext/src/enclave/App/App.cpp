 ...
 * annotated by cyx
 *
 * sgx_uae_service.h is deprecated;
 * replaced by sgx_uae_launch.h sgx_uae_epid.h sgx_uae_quote_ex.h
 */
...
static sgx_errlist_t sgx_errlist[] = {
  /**
   * modified by cyx
   * error related to remote Attestation
   */
  {
...
JNIEXPORT jbyteArray JNICALL Java_edu_xjtu_cs_cyx_qshield_execution_QShieldSGXEnclave_ResPrepared
  (JNIEnv *env, jobject obj, jlong eid, jbyteArray input_rows){
    ...
    if (input_rows_ptr == nullptr) {
      ocall_throw("ResPrepared: JNI failed to get input byte array.");
    } else {
      sgx_check_and_time("ResPrepared",
                         ecall_res_prepared(
                           eid,
                           input_rows_ptr, input_rows_length,
                           &output_rows, &output_rows_length));
...
JNIEXPORT jbyteArray JNICALL Java_edu_xjtu_cs_cyx_qshield_execution_QShieldSGXEnclave_QProject
  ...
  if (input_rows_ptr == nullptr) {
    ocall_throw("QProject: JNI failed to get input byte array.");
  } else {
    sgx_check_and_time("QProject",
                       ecall_qproject(
                         eid,
                         project_list_ptr, project_list_length,
                         input_rows_ptr, input_rows_length,
...
JNIEXPORT jbyteArray JNICALL Java_edu_xjtu_cs_cyx_qshield_execution_QShieldSGXEnclave_QFilter
  ...
  if (input_rows_ptr == nullptr) {
    ocall_throw("QFilter: JNI failed to get input byte array.");
  } else {
    sgx_check_and_time("QFilter",
                       ecall_qfilter(
                         eid,
                         condition_ptr, condition_length,
                         input_rows_ptr, input_rows_length,
...
JNIEXPORT jbyteArray JNICALL Java_edu_xjtu_cs_cyx_qshield_execution_QShieldSGXEnclave_QScanCollectLastPrimary
  ...
  if (input_rows_ptr == nullptr) {
    ocall_throw("QScanCollectLastPrimary: JNI failed to get input byte array.");
  } else {
    sgx_check_and_time("Q Scan Collect Last Primary",
                       ecall_qscan_collect_last_primary(
                         eid,
                         join_expr_ptr, join_expr_length,
                         input_rows_ptr, input_rows_length,
                         &output_rows, &output_rows_length));
...
JNIEXPORT jbyteArray JNICALL Java_edu_xjtu_cs_cyx_qshield_execution_QShieldSGXEnclave_QSortMergeJoin
  (JNIEnv *env, jobject obj, jlong eid, jbyteArray join_expr, jbyteArray input_rows, jbyteArray join_row){
  (void)obj;
  ...
  if (input_rows_ptr == nullptr) {
    ocall_throw("QSortMergeJoin: JNI failed to get input byte array.");
  } else {
    sgx_check_and_time("Q Sort-Merge Join",
                       ecall_qsort_merge_join(
                         eid,
                         join_expr_ptr, join_expr_length,
                         input_rows_ptr, input_rows_length,
                         join_row_ptr, join_row_length,
